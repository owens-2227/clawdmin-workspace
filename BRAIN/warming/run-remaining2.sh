...
ACCOUNTS=("k1bc5662|lhdqpdftdt" "k1bc56nd|vglmtlyrdm" "k1bc5757|cuvuvcljco")

for ACCT in "${ACCOUNTS[@]}"; do
  IFS='|' read -r PROF_ID UNAME <<< "$ACCT"
  echo ""
  echo "───── $UNAME (Day 4, Phase 2) ─────"
  
  RESP=$(curl -s "$API/browser/start?user_id=$PROF_ID" -H "$AUTH")
...
")
  ...
  # Use perl alarm instead of timeout (macOS)
  if [ -n "$SUBS" ]; then
    perl -e 'alarm 300; exec @ARGV' python3 "$WARM_SCRIPT" "$CDP" "$UNAME" 2 --subreddits "$SUBS" > "$LOG_FILE" 2>&1 || true
  else
    perl -e 'alarm 300; exec @ARGV' python3 "$WARM_SCRIPT" "$CDP" "$UNAME" 2 > "$LOG_FILE" 2>&1 || true
  fi
...
"
  ...
  curl -s "$API/browser/stop?user_id=$PROF_ID" -H "$AUTH" > /dev/null 2>&1
  sleep 25
done
